...
function createChoroplethMap(data) {
  ...
  avgUnemployment = data.reduce((acc, dataPoint) => {
    ...
    unemploymentYears.forEach((year) => {
      if (
        year >= timelineStartYear.getFullYear() &&
        year <= timelineEndYear.getFullYear()
      ) {
        sum += +dataPoint[year];
        count++;
      }
  ...
  mapSVG
    ...
    .on("mouseup", function (d) {
      ...
      if (countryIsInListOfCountries(countryName, "selection"))
        removeFromListOfCountries(countryName, "selection");
      else {
  ...
  linearGradient
    .selectAll("stop")
    .data(
      colorScale.ticks().map((d, i, nodes) => ({
...
function updateChoroplethMap() {
  ...
  // update legend text
  legendTextTop.text(maxAvg.toFixed(0) + "%");
  legendTextBottom.text(minAvg.toFixed(0) + "%");
